            ...
            <div class="card bg-base-100 shadow-lg">
              <div class="card-body">
                  ...
                  <ul class="space-y-4 my-4">
                    ...
                    <li class="flex justify-between items-center">
                      <div class="flex items-center">
                        <img [src]="item.product.image_link" [alt]="item.product.name" class="w-16 h-16 rounded-lg mr-4 object-cover" />
                        <div>
                          <p class="font-semibold">{{ item.product.name }}</p>
                          <p class="text-sm text-base-content/70">Qty: {{ item.quantity }}</p>
                        </div>
                      </div>
                      <p class="font-semibold">\${{ (parseFloat(item.product.price || '0') * item.quantity).toFixed(2) }}</p>
                    </li>
                    }
                  </ul>
                }
                ...
                <div class="space-y-2 mt-4">
                  <div class="flex justify-between">
                    <span>Subtotal</span><span>\${{ (subtotal$ | async)?.toFixed(2) }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span>Shipping</span><span>\${{ (shippingCost$ | async)?.toFixed(2) }}</span>
                  </div>
                  <div class="flex justify-between font-bold text-lg">
                    <span>Total</span><span>\${{ (total$ | async)?.toFixed(2) }}</span>
                  </div>
                </div>
                </div>
            </div>
